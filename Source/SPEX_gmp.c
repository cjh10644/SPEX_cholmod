//------------------------------------------------------------------------------
// SPEX_CHOLMOD/SPEX_gmp.c: interface to the gmp library
//------------------------------------------------------------------------------
...
// GMP library used by SPEX_CHOLMOD.  The wrappers enable memory failures to be
...
// application.  Fortunately, GMP allows the user package (SPEX_CHOLMOD in this
...
// functions, but just the ones used by SPEX_CHOLMOD.  However, most of the wrapper
...
)
{
    ...
    // called in SPEX_CHOLMOD_final ( ), it has to be called here to
