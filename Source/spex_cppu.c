...
)
{
    ...
    if (ks == n)
    {
        ...
        else
        {
            ...
            SPEX_CHECK(SPEX_mpz_swap(Uk_dense_row[Q[n-1]], U->v[k]->x[0]));
            U->v[k]->i[0] = Q[n-1];
            U->v[k]->nz = 1;

            // update sd[n-1], d[n-1], S(1,n-1)
