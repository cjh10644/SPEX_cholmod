...
)
{
    ...
    while (k < n-1)
    {
        ...
        for(int64_t ii = 0; ii<n;ii++)
        {
            SPEX_CHECK(SPEX_gmp_printf("%Qd*%Qd ", SPEX_2D(S, 1, ii),
                                                   SPEX_2D(S,3,ii)));
        }
        printf("]);\nu%ld=[\n",k);
        for (int64_t ii =0;ii<n;ii++)
        ...
        for(int64_t ii = 0; ii<n;ii++)
        {
            SPEX_CHECK(SPEX_gmp_printf("%Qd*%Qd ", SPEX_2D(S, 2, ii),
                                                   SPEX_2D(S, 3, ii)));
        }
        printf("])*u%ld;\nsd%ld=[",k,k);
    ...
    // S(:,k)=[1;1;1]
    SPEX_CHECK(SPEX_mpq_set_ui(SPEX_2D(S, 1, k), 1, 1));
    ...
    SPEX_FREE_ALL;
    return SPEX_OK;//result?SPEX_OK:SPEX_INCORRECT_INPUT;
}
